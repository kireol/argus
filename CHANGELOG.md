...
- **`--skip-to` matches console `N/M`.** Tests whose `platforms:` have no
  ...
  line, so resume landed earlier than the failed test number (VM-EV-006 at
  179/306 vs `--skip-to 179` running VM-EV-004). Those tests are omitted
...
- **`text_present` for `0%` / `75%`.** OCR digit confusions (O→0, …) only ran
  ...
  read as `O%` (VM-EV-002). Confusions now apply whenever the needle contains
  a digit. `%` is kept during punctuation normalize so `0%` does not match a
  bare `0 km`. Only Argus is bumped (Test Creator stays 1.2.8).
