...
CONFIGS = sorted(
    (ROOT / "examples").glob("*/argus*.yaml"),
    key=lambda p: (p.parent.name, p.name),
)


@pytest.mark.parametrize("config_path", CONFIGS, ids=lambda p: f"{p.parent.name}/{p.name}")
def test_example_config_and_tests_load(config_path, monkeypatch):
    example = config_path.parent
