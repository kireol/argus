...
2. User configuration (``utf init`` creates it)
...
def default_user_config_path() -> Path:
    ...
    return user_config_path("utf", appauthor=False) / "config.yaml"
    ...
    if config_file is not None:
        ...
        if not explicit.is_file():
            raise ConfigurationError(
                ...
                remediation="Check the --config path, or run 'utf init' to create one.",
            )
