...
from utf.config import default_user_config_path, load_config
...
from utf.logging import configure_logging
from utf.models.results import RunStatus
from utf.reporting import (
    ConsoleReporter,
    write_html_report,
    write_json_report,
    write_junit_report,
)
from utf.reporting.alerts import Alert, AlertSeverity, ConsoleAlertProvider
...
def main(
    ...
    state.quiet = quiet
    if version:
        console.print(f"utf {__version__}")
        raise typer.Exit(0)
    if dry_run:
        _dry_run()
        raise typer.Exit(0)
...
def validate(
    ...
    from utf.cli.validate import print_report, validate_environment
...
def init(
    ...
    from utf.cli.templates import CONFIG_TEMPLATE
