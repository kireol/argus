...
def test_skips_android_only_images_when_only_cpp_is_configured():
    ...
    check = TestAssetsCheck(
        session,
        [
            _test("DEMO-001", ["android"], "home_screen.png"),
            _test("TS-001", ["android", "cpp"], "wifi_icon_on.png"),
        ],
    )
    result = check.run()
    assert result.passed is False
    assert "wifi_icon_on.png" in (result.error or "")
    assert "home_screen.png" not in (result.error or "")
    assert result.diagnostics["checked_tests"] == 1


def test_passes_when_runnable_assets_exist():
    session = _session(
        platforms={"fallback": "cpp"}, present={"wifi_icon_on.png"}
    )
    check = TestAssetsCheck(
        session,
        [
