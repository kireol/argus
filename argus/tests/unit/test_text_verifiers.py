...
def test_text_present_ignores_trailing_punctuation():
    verifier = TextPresentVerifier(_FixedOCR("Charge Device Now"))
    result = verifier.verify(
        _observation(),
        Expectation(text="Charge Device Now!", region=Region(x=0, y=0, width=1, height=1)),
    )
...
def test_text_not_present_ignores_trailing_punctuation():
    verifier = TextAbsentVerifier(_FixedOCR("Charge Device Now"))
    result = verifier.verify(
        _observation(),
        Expectation(text="Charge Device Now!", region=Region(x=0, y=0, width=1, height=1)),
    )
